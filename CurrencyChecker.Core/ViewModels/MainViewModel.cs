...
namespace CurrencyChecker.Core.ViewModels
{
    public class MainViewModel : BaseViewModel
    {
        ...
        private readonly List<RemoteRateViewModel> _sourceItems = new List<RemoteRateViewModel>();
        public ObservableCollection<RemoteRateViewModel> Items { get; set; }
        public IAsyncCommand RefreshCommand { get; }
        public IAsyncCommand<RemoteRateViewModel> ItemTappedCommand { get; }
        ...
        public MainViewModel(IExternalCurrencyService currencyService, ISettingsProvider settingsProvider, IErrorHandler errorHandler, ILocalCurrencyService localCurrencyService)
        {
            ...
            Items = new ObservableCollection<RemoteRateViewModel>();
            ...
            ItemTappedCommand = new AsyncCommand<RemoteRateViewModel>(ItemTapped, errorHandler: errorHandler);
        ...
        private async Task ItemTapped(RemoteRateViewModel vm)
        {
            ...
            try
            {
                ...
                if (result?.Rates != null)
                    foreach (var item in result.Rates)
                        _sourceItems.Add(new RemoteRateViewModel(result.Base ?? "", item.Key, item.Value, _currencyService, _settingsProvider, _localCurrencyService, _errorHandler));
